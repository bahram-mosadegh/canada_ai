...
function sidenavTypeOnResize() {
  ...
  if (window.innerWidth < 1200) {
    ...
    // });
    $('.display-sidebar').hide();
  } else {
    // elements.forEach(function(el) {
    //   el.classList.remove('disabled');
    // });
    $('.display-sidebar').show();
  }
}
...
function alphabet_check(event) {
    return (event.charCode >= 1574 && event.charCode <= 1594) || (event.charCode >= 1601 && event.charCode <= 1610) || (event.charCode == 1570) || (event.charCode == 1662) || (event.charCode == 1670) || (event.charCode == 1688) || (event.charCode == 32) || (event.charCode == 1705) || (event.charCode == 1711) || (event.charCode == 1740) || (event.charCode == 1572)
}

function final_alphabet_check(str) {
    return /^[پچجحخهعغفقثصضگکمنتالبیسشوئءدذرزژطظ آءيكةؤ]+$/i.test(str);
}

function final_numeric_check(str) {
    return /^[0-9]+$/i.test(str);
}

function latin_chars_check(event) {
    return (event.charCode >= 65 && event.charCode <= 90) || (event.charCode == 32) || (event.charCode >= 97 && event.charCode <= 122)
}

function passport_chars_check(event) {
    return (event.charCode >= 65 && event.charCode <= 90) || (event.charCode >= 97 && event.charCode <= 122) || (event.charCode >= 48 && event.charCode <= 57)
}

function final_latin_name_check(str) {
    return /^[a-z ]+$/i.test(str);
}

function final_passport_check(str) {
    return /^[a-z0-9]+$/i.test(str);
}

function final_date_check(str) {
    return /^[0-9\/]+$/i.test(str);
}

function final_time_check(str) {
    return /^([0-1]\d|2[0-3]):[0-5]\d$/.test(str);
}

function final_shamsi_date_check(str) {
    return /^1\d\d\d\/(0\d|1[0-2])\/([0-2]\d|3[0-1])$/.test(str);
}
...
$(document).on('keypress', '.number-separator', function (e) {
    if (/^-?\d*[,.]?(\d{0,3},)*(\d{3},)?\d{0,3}$/.test(e.key)) {
        $('.number-separator').on('input', function () {
          e.target.value = number_separator(e.target.value);
        });
    } else {
        e.preventDefault();
        return false;
...
function parent_check_box(element, action_classes = []) {
    ...
    $('.child-check-box').prop('checked', isChecked);
    $('.parent-check-box').prop('checked', isChecked);
    action_classes.forEach(item => {
      $('.'+item).prop('disabled', !(isChecked && $('.parent-check-box').length));
    });
};

function child_check_box(action_classes = []) {
    const allChecked = $('.child-check-box:checked').length === $('.child-check-box').length;
    $('.parent-check-box').prop('checked', allChecked);
    action_classes.forEach(item => {
...
$("form").submit(function() {
  ...
  $("button[type='submit'][form='"+$(this).attr('id')+"']").attr('disabled', true);
});
...
if($('#dropzone').length) {
    ...
    var myDropzone = new Dropzone(drop, {
        maxFilesize: 50,
        acceptedFiles: '.jpeg,.jpg,.png,.pdf,.xls,.xlsx,.doc,.docx,.mp4',
        addRemoveLinks: true,
        dictDefaultMessage: 'فایل  ها را اینجا رها یا Paste کنید تا آپلود شوند',
        dictRemoveFile: 'حذف',
        dictCancelUpload: 'لغو آپلود',
        dictCancelUploadConfirmation: 'آیا مطمئنید که می خواهید این آپلود را لغو کنید؟',
        dictFileTooBig: 'حجم فایل زیاد است. حداکثر حجم مجاز {{maxFilesize}}MB است.',
        dictInvalidFileType: 'شما نمی توانید این نوع فایل را آپلود کنید.',
        removedfile: function(file) {
            if (file.upload && $('#'+file.upload.uuid).length) {
                $('#'+file.upload.uuid).remove();
            } else {
                $('#'+file.name.split(".")[0]).remove();
            }
            var _ref;
    ...
    myDropzone.on('success', function(file, response) {
        if (response.success) {
            $('#attachments').append('<input hidden type="text" name="attachments[]" value="'+response.file_name+'" id="'+file.upload.uuid+'">');
        } else {
    ...
    $(document).ready(function () {
        $('[data-dz-name]').on('click', function () {
